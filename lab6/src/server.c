...
int main(int argc, char **argv) {
    ...
    while (true) {
        ...
        while (true) {
            ...
            unsigned int num_threads = (unsigned int)tnum;
            ...
            if (begin > end || mod == 0) {
                fprintf(stderr, "Invalid range: begin=%"PRIu64", end=%"PRIu64", mod=%"PRIu64"\n", 
                        begin, end, mod);
                break;
            }

            struct FactorialArgs args[num_threads];
            uint64_t numbers_per_thread = (end - begin + 1) / num_threads;
            uint64_t remainder = (end - begin + 1) % num_threads;
            ...
            for (unsigned int i = 0; i < num_threads; i++) {
                ...
                printf("Thread %u: numbers from %"PRIu64" to %"PRIu64"\n", 
